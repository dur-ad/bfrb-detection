...
# Input/Output paths
VIDEO_ROOT = Path("C:\\Users\\desk-7\\Documents\\PRROCESSING-bfrb_videos")  # Original video folder
COORDS_ROOT = Path("C:\\Users\\desk-7\\Documents\\coords")        # Intermediate landmarks folder
BFRB_ROOT = Path("C:\\Users\\desk-7\\Documents\\final_coords (S8,S10,S13)")         # Final BFRB features folder
...
# MEDIAPIPE LANDMARKS
#
# The code supports both the legacy `mp.solutions` API and the newer
# MediaPipe Tasks API (packaged under `mediapipe.tasks`). If the Tasks API is
# present, a thin wrapper (`TasksHolistic`) creates a PoseLandmarker-based
# processor and exposes a `process(image_rgb)` method that returns an object
# with a `pose_world_landmarks` attribute to keep the rest of the pipeline
# unchanged. The Tasks API requires a `.task` model file; set its path via
# the `POSE_LANDMARKER_MODEL_PATH` environment variable.
# =============================================================================

mp_holistic = None

def _create_tasks_holistic_wrapper(model_asset_path=None,
                                   min_detection_confidence=0.5,
                                   min_tracking_confidence=0.5):
    try:
        # Import the Tasks API modules (packaged under mediapipe.tasks.python)
        from mediapipe.tasks.python import vision as mp_vision
        from mediapipe.tasks.python.vision import pose_landmarker as pl
    except Exception as e:
        raise ImportError(
            "MediaPipe Tasks API not available. Install a mediapipe build that provides "
            "the Tasks API or set up the legacy `mp.solutions` API.") from e

    # Determine model asset path: allow override via environment variable
    model_asset_path = model_asset_path or os.environ.get("POSE_LANDMARKER_MODEL_PATH")

    # If not provided or not found, attempt to download a default model
    if not model_asset_path or not os.path.exists(model_asset_path):
        try:
            model_asset_path = _download_default_pose_landmarker()
            # export for subsequent runs
            os.environ["POSE_LANDMARKER_MODEL_PATH"] = str(model_asset_path)
        except Exception:
            raise FileNotFoundError(
                "Pose Landmarker task model not found. Set POSE_LANDMARKER_MODEL_PATH to a valid 'pose_landmarker_*.task' file path, or ensure the script can download the default model.")

    # Configure BaseOptions and PoseLandmarkerOptions
    base_options = pl.BaseOptions(model_asset_path=model_asset_path)
    options = pl.PoseLandmarkerOptions(
        base_options=base_options,
        running_mode=pl.VisionRunningMode.VIDEO,
        min_pose_detection_confidence=min_detection_confidence,
        min_pose_tracking_confidence=min_tracking_confidence,
    )

    landmarker = pl.PoseLandmarker.create_from_options(options)
    ...
    class TasksHolistic:
        ...
        def process(self, rgb_image):
            # Convert numpy RGB image -> mediapipe.tasks.python.vision.Image
            mp_image = mp_vision.Image.create_from_array(rgb_image)

            # Provide a timestamp in milliseconds; monotonic increasing values
            timestamp_ms = int(self._frame_idx)
            result = self._landmarker.detect_for_video(mp_image, timestamp_ms)

            # Wrap results to provide a `pose_world_landmarks` attribute similar
            # to the legacy `Holistic` results object used by the remainder
            # of the pipeline.
            out = SimpleNamespace()
            # result.pose_world_landmarks may be a list (multi-person) or None
            pw = getattr(result, 'pose_world_landmarks', None)
            if pw:
                # If a list-like, take the first person; otherwise pass through
                try:
                    out.pose_world_landmarks = pw[0]
                except Exception:
                    out.pose_world_landmarks = pw
            else:
                out.pose_world_landmarks = None
        ...
        def close(self):
            try:
                # underlying landmarker may expose a close() method
                self._landmarker.close()
            except Exception:
                pass

    return TasksHolistic(landmarker)


def _download_default_pose_landmarker(dest_dir=None):
    """Attempt to download a default MediaPipe Pose Landmarker .task model.

    Returns the local path to the downloaded model.
    """
    dest_dir = _Path(dest_dir or Path.cwd() / "mediapipe_models")
    dest_dir.mkdir(parents=True, exist_ok=True)

    candidates = [
        # Official MediaPipe task model storage (full and lite)
    ...
    raise RuntimeError("Failed to download a default Pose Landmarker .task model from known sources.")

# If legacy `mp.solutions` is available use it; otherwise we'll attempt to
# use the Tasks API at runtime when extracting landmarks.
...
def extract_landmarks_from_video(video_path, subject_id, activity_idx):
    ...
    # Initialize a processor that exposes a `process(rgb_image)` method and
    # a `close()` method. Prefer the legacy `mp.solutions` Holistic if present,
    # otherwise create a Tasks-based wrapper (requires POSE_LANDMARKER_MODEL_PATH).
    if mp_holistic is not None:
        ...
        try:
            holistic = _create_tasks_holistic_wrapper(
                model_asset_path=os.environ.get('POSE_LANDMARKER_MODEL_PATH'),
                min_detection_confidence=0.5,
                min_tracking_confidence=0.5,
            )
        except Exception as e:
            print(f"Could not initialize MediaPipe Tasks PoseLandmarker: {e}")
            cap.release()
            return None

...
INPUT_ROOT = Path("C:\\Users\\desk-7\\Documents\\S11")
OUTPUT_ROOT = Path("C:\\Users\\desk-7\\Documents\\output")
...
def process_video(video_path, subject_id, activity_idx):
    ...
    try:
        while True:
            ...
            # track if any landmarks were detected at all
    ...
    if not detected_any:
        print("mediapipe is not running on the video")
...
def main():
    ...
    for subject_dir in sorted(INPUT_ROOT.iterdir()):
        ...
        # Support two layouts:
        # 1) subject/activity dirs (original): <subject>/<something-<idx>>/*.mov
        # 2) flat layout: videos directly inside subject folder

        # First handle activity subdirectories as before
        for act_dir in sorted(subject_dir.iterdir()):
            # if it's a directory, try to parse activity index and process its videos
            if act_dir.is_dir():
                try:
        ...
        # Then also handle videos directly inside the subject folder (no activity dirs)
        for video in sorted(subject_dir.iterdir()):
            if video.is_file() and video.suffix.lower() in VIDEO_EXTS:
                # activity unknown; use 0 which will yield Class_0
                activity_idx = 0
                print("   Video:", video.name)
                process_video(video, subject_id, activity_idx)
